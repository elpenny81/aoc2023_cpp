...
QString part1(const QString &fileName) {
    ...
    return process([&maxScores](quint32 gameId, const QString &game) {
        const auto scores = maxScoresFromGame(game);
        if (scores[Color::red] <= maxScores[Color::red] &&
             scores[Color::green] <= maxScores[Color::green] &&
...
QString part2(const QString &fileName) {
    return process([](quint32, const QString &game) {
        const auto scores = maxScoresFromGame(game);
